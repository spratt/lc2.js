...
var LC2 = (function(LC2, undefined) {
    ...
    var hex_pattern = /^(?:\$|x|X|0x|0X)(-?[\da-fA-F]+)/;
    var bin_pattern = /^[%bB](-?[01]+)/;
    var dec_pattern = /^#?(-?\d+)/;
    
    LC2.parseNum = function(num_str) {
        var hex = hex_pattern.exec(num_str);
        var bin = bin_pattern.exec(num_str);
        var dec = dec_pattern.exec(num_str);
